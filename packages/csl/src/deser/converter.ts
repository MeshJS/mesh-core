...
export const toRedeemer = (action: Action) => {
    const lookupRedeemerTag = (key: keyof typeof REDEEMER_TAGS): RedeemerTag => REDEEMER_TAGS[key];

    return csl.Redeemer.new(
        lookupRedeemerTag(action.tag as keyof typeof REDEEMER_TAGS),
    ...
    switch (relay.type) {
        ...
        case 'SingleHostName':
            return csl.Relay.new_single_host_name(
                csl.SingleHostName.new(relay.port, csl.DNSRecordAorAAAA.new(relay.domainName)),
            );
        case 'MultiHostName':
    ...
    if (scriptRef.is_plutus_script()) {
        ...
        return <PlutusScript>{
            ...
            version: Object.keys(LANGUAGE_VERSIONS).find(
                (key) =>
                    LANGUAGE_VERSIONS[key as keyof typeof LANGUAGE_VERSIONS].to_hex() ===
...
export const fromTxUnspentOutput = (txUnspentOutput: TransactionUnspentOutput) => {
...
export const fromValue = (value: Value) => {
    const assets: Asset[] = [{ unit: 'lovelace', quantity: value.coin().to_str() }];

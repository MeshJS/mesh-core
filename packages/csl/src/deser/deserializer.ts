...
export const deserializeAddress = (address: string) => csl.Address.from_bytes(toBytes(address));

export const deserializeBip32PrivateKey = (bip32PrivateKey: string) =>
    csl.Bip32PrivateKey.from_bytes(toBytes(bip32PrivateKey));

export const deserializeDataHash = (dataHash: string) => csl.DataHash.from_bytes(toBytes(dataHash));
...
export const deserializePlutusScript = (plutusScript: string, version: LanguageVersion) =>
    csl.PlutusScript.from_bytes_with_version(toBytes(plutusScript), LANGUAGE_VERSIONS[version]);
...
export const deserializeTx = (tx: string) => csl.Transaction.from_bytes(toBytes(tx));
...
export const deserializeValue = (value: string) => csl.Value.from_bytes(toBytes(value));

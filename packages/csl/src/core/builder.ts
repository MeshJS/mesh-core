...
export class CSLSerializer implements IMeshSerializer {
    ...
    private makePlutusScriptSource = (
        ...
        const scriptSource = csl.PlutusScriptSource.new_ref_input_with_lang_ver(
            scriptHash,
            scriptRefInput,
            LANGUAGE_VERSIONS[scriptSourceInfo.version as 'V1' | 'V2'],
        );
        return scriptSource;
    };
    ...
    private addScriptTxIn = ({
        ...
        if (scriptSource.type === 'Inline') {
            ...
            cslScript = csl.PlutusScriptSource.new(
                csl.PlutusScript.from_hex_with_version(
                    ...
                    LANGUAGE_VERSIONS[scriptSource.script.version as 'V1' | 'V2'],
                ),
            );
        }
        ...
        this.txBuilder.add_plutus_script_input(
            ...
            csl.TransactionInput.new(csl.TransactionHash.from_hex(txIn.txHash), txIn.txIndex),
            toValue(txIn.amount),
        );
    };
    ...
    private addOutput = ({ amount, address, datum, referenceScript }: Output) => {
        ...
        if (txValue.is_zero() && multiAsset === undefined) throw Error('Invalid output amount');
            ...
            outputBuilder = outputBuilder.with_script_ref(
                csl.ScriptRef.new_plutus_script(
                    csl.PlutusScript.from_hex_with_version(
                        ...
                        LANGUAGE_VERSIONS[referenceScript.version as 'V1' | 'V2'],
                    ),
                ),
            );
        }
    ...
    protected addAllMints = (mints: MintItem[]) => {
        ...
        for (let i = 0; i < mints.length; i += 1) {
            ...
            if (!mintItem.scriptSource) throw Error('Mint script is expected to be provided');
        ...
        const script =
            scriptSource.type === 'Reference Script'
                ? csl.PlutusScriptSource.new_ref_input_with_lang_ver(
                      ...
                      LANGUAGE_VERSIONS[scriptSource.version as 'V1' | 'V2'],
                  )
    ...
    private addNativeMint = (
        ...
        mintBuilder.add_asset(
            csl.MintWitness.new_native_script(csl.NativeScript.from_hex(scriptSource.script.code)),
            csl.AssetName.new(Buffer.from(assetName, 'hex')),
            csl.Int.from_str(amount),
        );
    };
    ...
    private addCertificate = (certificates: csl.Certificates, cert: Certificate) => {
        switch (cert.type) {
            case 'RegisterPool':
                certificates.add(
    ...
    private addValidityRange = ({ invalidBefore, invalidHereafter }: ValidityRange) => {
        if (invalidBefore) {

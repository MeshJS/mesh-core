import { AddressUTXO, BlockFrostOutput, OutputAmount } from "src/type/blockfrost";
import csl from "@emurgo/cardano-serialization-lib-nodejs"

export const getOutputLovelace = <T extends { amount?: { unit?: string, quantity?: string }[] }>(output: T): string => {
  ...
  bfData.forEach(blockfrostUtxo => {
    ...
    const cslUtxo = csl.TransactionUnspentOutput.new(
      ...
      csl.TransactionOutput.new(
        csl.Address.from_bech32(blockfrostUtxo.address),
        cslValue
      )
    )
